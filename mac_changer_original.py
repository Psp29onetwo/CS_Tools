...
def get_args():
  parser = argparse.ArgumentParser(description="Please use this tool with root privileges.")
...
if changed_mac == command_args.new_mac:
  ...
  # Asking user to reset the system after changing MAC adress
  if_user_want_a_system_restart = input("System needs a restart \nDo you want to restart you system? [y/Y/Yes] Yes or [n/N/No] No: ")
  accepted_strings = {'y', 'yes'}
  if if_user_want_a_system_restart.lower() in accepted_strings:
    sub.call(["sudo", "reboot"])
else:
